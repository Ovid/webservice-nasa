...
This is an internal fork for L<Data::Walk>. The behavior is almost identical, but
we sort hash keys to ensure deterministic walking.

The above synopsis bears an amazing similarity to File::Find(3pm)
and this is not coincidental.

WebService::NASA::DataWalk(3pm) is for data what File::Find(3pm) is for files.
...
to preprocess the current node $WebService::NASA::DataWalk::container.  Your
...
then.  The variable $WebService::NASA::DataWalk::type will contain the string
...
with no arguments.  The variable $WebService::NASA::DataWalk::container points
...
function is a generic callback and does B<not> tell WebService::NASA::DataWalk(3pm)
...
=item B<$WebService::NASA::DataWalk::container>
...
=item B<$WebService::NASA::DataWalk::type>

The base type of the object that $WebService::NASA::DataWalk::container
references.  This is either "ARRAY" or "HASH" or the empty string for
everything else.

=item B<$WebService::NASA::DataWalk::seen>
...
=item B<$WebService::NASA::DataWalk::address>
...
=item B<$WebService::NASA::DataWalk::depth>

The depth of the current recursion.

=item B<$WebService::NASA::DataWalk::index>
...
a hash and B<$WebService::NASA::DataWalk::index> is even then B<$_> is a hash key.  If
...
This variable has been added in WebService::NASA::DataWalk version 1.01.
...
The API of WebService::NASA::DataWalk(3pm) tries to mimic the API of File::Find(3pm)
to a certain extent.  If you are already familiar with File::Find(3pm) 
you will find it very easy to use WebService::NASA::DataWalk(3pm).  Even the
documentation for WebService::NASA::DataWalk(3pm) is in parts similar or identcal
to that of File::Find(3pm).

=head2 Analogies

The equivalent of directories in File::Find(3pm) are the container
data types in WebService::NASA::DataWalk(3pm).  Container data types are arrays
...
WebService::NASA::DataWalk(3pm) passes lists of variables.

=head2 Function Names

Instead of C<find()> and C<finddepth()>, WebService::NASA::DataWalk(3pm) uses 
C<walk()> and C<walkdepth()>, like the smart reader 
has already guessed after reading the L</"SYNOPSIS">.

=head2 Variables

The variable $WebService::NASA::DataWalk::container is vaguely equivalent to 
...
WebService::NASA::DataWalk(3pm) is called with $_ set to the currently inspected
item.

=head2 Options

The option I<follow> has the effect that WebService::NASA::DataWalk(3pm) also
...
are ignored by WebService::NASA::DataWalk(3pm).  Examples are I<follow_fast>,
...
If you want to stop the recursion at a certain level, do it as follows:
    ...
    sub not_too_deep {
        if ( $WebService::NASA::DataWalk::depth > $max_depth ) {
            return ();
        }
        else {
            return @_;
